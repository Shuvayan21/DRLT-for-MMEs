function [Sens_delta, Spec_delta, Sens_beta, Spec_beta, RRMSE] = results_Sens_Spec_RRMSE_viral_loads(A_tilde, A, beta, delta, sigma, W)
    ...
    RRMSE = zeros(3, run); % RRMSE results for different methods.
    Sens_beta = zeros(3, run); % Sensitivity for beta.
    Spec_beta = zeros(3, run); % Specificity for beta.
    ...
    for k = 1:run
        ...
        cvx_end
        RRMSE(1, k) = norm(beta - x_l(1:p)) / norm(beta); % Store RRMSE for robust lasso.
        ...
        cvx_end
        RRMSE(2, k) = norm(beta - x_odrl(1:p)) / norm(beta); % Store RRMSE for ODRL.
        ...
        cvx_end
        RRMSE(3, k) = norm(beta - x_drl(1:p)) / norm(beta); % Store RRMSE for DRLT.
    end
end
