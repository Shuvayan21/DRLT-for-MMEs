...
% This function computes a weight matrix W based on the input design matrix A
    ...
    mu_1 = 2 * h^2 * sqrt(2 * log(p) / n); % Upper bound for the first constraint.
    mu_2 = 4 * h^3 * sqrt(log((2 * p * n)) / (p * n)) + 1 / n; % Upper bound for the second constraint.
    mu_3 = 2 * h^2 * sqrt(2 * log(n) / p) / sqrt(1 - n / p); % Upper bound for the third constraint.
    ...
    cvx_begin 
        ...
        subject to
            ...
            -mu_3 * Ones_nn <= n / (p * sqrt(1 - n / p)) * (A * W' / n - p * I_n / n) <= mu_3 * Ones_nn;
