function [rmse] = ransac_l2(y, A, x, lam, itr)    
    % RANSAC_L2_CONFUSION performs the RANSAC algorithm to fit a model using L2 regularization
    % and computes the RMSE compared to a true parameter vector.
    %
    % Inputs:
    ...
    k = ceil(0.9 * m); 
    good_test = 100; % Initialize the threshold for a good test error
    ...
    for j = 1:1:itr
        ...
        cvx_begin
            ...
            minimise (pow_pos(norm(y_train - A_train * the), 2) + lamb * norm(the, 1)) 
            subject to
                for i = 1:1:n
                    the(i) >= 0; % Ensure non-negativity constraint on coefficients
                end
        cvx_end
    ...
    cvx_begin
        ...
        minimise (pow_pos(norm(y_new - A * thez), 2) + lamb * norm(thez, 1)) 
        subject to
            for i = 1:1:n
                thez(i) >= 0; % Ensure non-negativity constraint on coefficients
            end
    cvx_end
